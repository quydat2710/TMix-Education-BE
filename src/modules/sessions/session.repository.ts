...
export class SessionRepository {
  constructor(
    ...
    private attendanceSessionRepository: Repository<AttendanceSessionEntity>,
  ...
  async updateAttendanceSession(
    ...
    const cases = payload
      .map((item) => `WHEN ${parseInt(item.studentId)} THEN '${item.status}'`)
      .join(' ');

    const updateRes = await this.attendanceSessionRepository
      .createQueryBuilder()
      .update()
    ...
    // Auto-update teacher payment when attendance is taken
    this.teacherPaymentsService.autoUpdatePayment(entity);
    return updateRes;
  }

  async getStudentAttendance(studentId: Student['id']) {
    const studentAttendance = await this.sessionRepository
